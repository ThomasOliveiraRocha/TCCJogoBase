export default class quizz2 extends Phaser.Scene {
    constructor() {
        super('quizz2');
    }
    ...
    create() {
        ...
        const pergunta = 'Qual destas alternativas descreve corretamente um ataque de "Credential Stuffing"?';

        const alternativas = [
            
            { texto: 'Inserção de código malicioso em formulários para capturar dados.', correta: false },
            { texto: 'Negação de serviço através do envio massivo de requisições.', correta: false },
            { texto: 'Intercepção de tráfego para escutar comunicações seguras.', correta: false },
            { texto: 'Uso de bots para tentar combinações de usuário e senha roubadas em vários sites.', correta: true },
            { texto: 'Comprometimento de fornecedores para atacar a cadeia de suprimentos.', correta: false },
        ];

        this.add.text(centerX, centerY - alturaCaixa / 2 + 30, 'Quiz 2: Segurança da Informação', {
            fontFamily: 'VT323',
            fontSize: '26px',
            fill: '#003366',
            stroke: '#ffffff',
        ...
        const proximoBtn = this.add.text(centerX, voltarMenu.y - 40, 'Próxima Pergunta →', {
            ...
            .on('pointerdown', () => this.scene.start('quizz3'));  // próxima cena
