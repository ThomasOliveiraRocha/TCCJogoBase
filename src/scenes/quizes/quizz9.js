export default class quizz9 extends Phaser.Scene {
    constructor() {
        super('quizz9');
    }
    ...
    create() {
        ...
        const pergunta = 'Qual das opções melhor representa um ataque do tipo "Watering Hole"?';

        const alternativas = [
            
            { texto: 'O invasor engana a vítima para baixar um software aparentemente legítimo.', correta: false }, // parece certo, mas é fake update
            { texto: 'A vítima recebe um e-mail falso com link malicioso para capturar credenciais.', correta: false }, // phishing clássico
            { texto: 'Usuários são redirecionados automaticamente por anúncios maliciosos sem clicar em nada.', correta: false }, // malvertising
            { texto: 'Criptografia dos dados do usuário após acesso a um arquivo desconhecido.', correta: false }, // ransomware
            { texto: 'O atacante compromete sites frequentemente acessados pela vítima para infectá-la.', correta: true }
        ];

        this.add.text(centerX, centerY - alturaCaixa / 2 + 30, 'Quiz 9: Segurança da Informação', {
            fontFamily: 'VT323',
            fontSize: '26px',
            fill: '#003366',
            stroke: '#ffffff',
        ...
        const proximoBtn = this.add.text(centerX, voltarMenu.y - 40, 'Próxima Pergunta →', {
            ...
            .on('pointerdown', () => this.scene.start('quizz10'));  // próxima cena
